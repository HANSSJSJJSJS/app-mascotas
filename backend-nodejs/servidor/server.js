  ...
  try {
    const [rows] = await pool.query(`
      SELECT 
        ...
        m.color,
        m.notas,
        m.ultima_visita as ultimaVisita,
        m.proxima_cita as proximaCita,
        m.vacunado,
        m.esterilizado,
        m.activo,
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO mascotas (
        ...
        color, notas, ultima_visita, proxima_cita, vacunado, esterilizado, activo
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        nombre, tipo, raza, edad, genero, peso, idPropietario,
        color, notas, ultimaVisita, proximaCita, vacunado, esterilizado, activo
      ]
    );
...
// Para manejar errores 404 (Rutas no encontradas)
app.use((req, res) => {
